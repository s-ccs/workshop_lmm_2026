...
begin
  ...
  for g in groupby(d_filtered, :participant_id)  # !!! → grouping variable
      _t = g.decisionTime
      m_mad = mad(_t)
      g.bad .= (_t .< (median(_t) - 3*m_mad)) .|| (_t .> (median(_t) + 3*m_mad))
  end
...
# ╔═╡ 541c840b-6815-4413-83f9-c64c1025b1b4
...
#f = @formula(!!! ~ 1 + !!! + (1 + !!! | !!!))
f = @formula(decisionTime ~ 1 + probability_abs + (1 + probability_abs | participant_id))
...
# ╔═╡ 03ab05bb-a3a1-4fa5-8ff9-e4f141637f58
...
question_box(
"""
This plot helps with interpretation of the random correlation: What is the meaning of the ρ - relationship between the intercept and the slope?
"""
)
...
begin
  ...
  #m0 = lmm(@formula(decisionTime ~ xxx, data_cean)
  #lrtest(!!!, !!!)
...
answer_box(md"""
  **Hint**:
  1. Fit the null model (remove `probability_abs` from fixed effects)
...
```
  m_main = lmm(@formula(decisionTime ~ probability_abs + arrangement + (1 + probability_abs*arrangement | participant_id)), d_clean)
  m_int = lmm(@formula(decisionTime ~ probability_abs * arrangement + (1 + probability_abs*arrangement | participant_id)), d_clean)
...
m_subset_simplified = lmm(@formula(decisionTime ~ arrangement*probability_abs + (1+arrangement*probability_abs|participant_id)),d_subset)
