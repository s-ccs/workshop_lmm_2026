...
begin
using CairoMakie, AlgebraOfGraphics, MixedModelsMakie, SwarmMakie
...
using Distributions
using DisplayAs
set_theme!(Theme(fontsize=14, font="Helvetica"))
end
...
1. Derive `probability_abs` (distance from 50%)
2. Filter to the relevant experimental conditions
3. Remove outliers using MAD-based detection
"""

# ╔═╡ 1c851de1-face-49a6-9a5c-61a1a5484d1b
  d.probability_abs = abs.((50 .- d.probability)) # higher value => easier decision

# ╔═╡ a2494d57-e9a1-4de4-9a82-24986a314ac4
 
	  answer_box("""
	  abs.((50 .- d.probability))
	  """)

# ╔═╡ 7ef66eeb-d5b9-4b49-a1b6-bbcfec7d319f
md"""
We want only  `denominator == 100` and non-natural-frequency conditions
"""

# ╔═╡ 43300a4a-8e83-4a70-a01a-7fcf92af49de
  d_filtered = subset(d, 
					  :denominator => x -> (x .== 100), 
					  :iconicity => x -> (x .!= "naturalFrequency")) ;


# ╔═╡ c8d3eca0-9bc4-424d-bfb8-167beef915f3
md"""
### A first visualization
"""

# ╔═╡ 20e0ce71-5c19-4271-b6d6-81c21f182eff
md"""
Generate a histogram of the decisionTime
"""

# ╔═╡ 47d184c7-a60f-4e4b-86c4-b19aa21bf538
data(d)*mapping(:decisionTime)*histogram(bins=10000)|>draw

# ╔═╡ 94e772e1-b47f-4d8e-9159-4426adff6b4a
question_box("""
What's up with this histogram? Are we good to go?			 
""")

# ╔═╡ 75f06df8-14d3-4ba8-899f-f3144e3ed934
answer_box(
    md"""
   No! There are huge outliers that will break our regression
    """
)

# ╔═╡ a9ccdfdb-e7b8-455d-86df-77f1a964f407
md"""
## Outlier Detection (MAD-based)

Let's detect the outliers per participant using MAD
"""

# ╔═╡ 9ebefb97-449f-4fe5-9f1b-b3c1191c4a25
PlutoTeachingTools.question_box(md"""
  **Task**: Find outliers in `decisionTime` using 3 * `StatsBase.mad`. Fill in the Lückentext below
  """)

# ╔═╡ 67e50e54-80e5-4f78-8499-914a5b819cd9
PlutoTeachingTools.hint(md"""
  **Hint**:
  - Use `groupby(d_filtered, :!!!)` to loop per participant
  - Use `mad(x)` (from `StatsBase`) for the MAD
  - Assign boolean: `x .< (median - 2*mad) .|| x .> (median + 2*mad)`

  """)

# ╔═╡ d65c72db-4e91-413f-82d5-a5695df71c47
begin
  d_filtered.bad .= false  # Initialize
	...
	d_clean = subset(d_filtered, :bad => .!);
	
	n_bad = sum(d_filtered.bad)
  	n_total = nrow(d_filtered)
  	"Outliers detected: $n_bad / $n_total ($(round(100*n_bad/n_total))%)"
end
...
begin
  d_filtered.bad .= false  # Initialize
  for g in groupby(d_filtered, :participant_id)  # !!! → grouping variable
      x = g.decisionTime
      m_mad = mad(x)
      g.bad .= (x .< (median(x) .- 2*m_mad)) .|| (x .> (median(x) .+ 2*m_mad))
  end
end
```""")
...
Start by visualizing the raw data.
"""

# ╔═╡ aba293d5-4e1e-4cf8-b1c8-375a56f2d0a1
PlutoTeachingTools.question_box(md"""
  **Task**: Create a plot of `probability_abs` vs `decisionTime`.

- Individual data points colored by `arrangement`
- A smoothed trend line with confidence intervals using `smooth`
- Bonus: You might want to use `Scatter` or `Beeswarm`

  The logic of AlgebraOfGraphics you should use:

`data(df) * mapping(:x,:y, color=:group) * (visual(Scatter) + smooth()) |> draw`
  """)

# ╔═╡ 3a0e29fa-aaaa-4c1c-bcd0-7ae9407da9b6
#data(...) * mapping(...) * (visual(...) + smooth()) |> draw

# ╔═╡ dc8fe453-c799-41d2-bddb-ddd50766ed69
PlutoTeachingTools.hint(md"""
**Hint**:
You could also use `visual(Beeswarm; algorithm=:quasirandom, alpha=0.1)` for nicer display!
  """)

# ╔═╡ 18a2929b-35f9-42ba-99ab-09cdd73b5107
answer_box("""
```julia
 data(d_clean) * mapping(:probability_abs, :decisionTime, color=:arrangement) *
      (visual(Beeswarm; algorithm=:quasirandom, alpha=0.1) + smooth())|> draw
```		   
""")

# ╔═╡ 1e7cb251-1dce-422b-b1b1-d6e0e1306005
# ---

question_box(md"""
1) Is there a relationship between probability distance and decision time?
2) Do the two arrangements (Ordered vs Random) show different patterns?
""")

# ╔═╡ babdb640-7c20-4fd7-a4dd-a616c71e7981
# ---

md"""
## Exercise 2: Building Your First Mixed Model

Now we are nearly read to fit a linear mixed model.
"""

# ╔═╡ 4f848403-37fd-4ef3-b664-aee71fa3e704
PlutoTeachingTools.question_box(md"""
We want to know the influence of probability_abs on decision-time.

in order to built our formula, we need to know:
								
- What are the **fixed effects** of interest?
- What are the **random grouping variables**?
- Do we need **random slopes**?
- Could random intercept and random slopes be correlated?
								
""")

# ╔═╡ 252fa588-7db7-4acf-a076-4cc4b540750f
answer_box(
"""
- `probability_abs` is for now our only fixed effect
- `participant_id` is the random grouping variable
- `probability_abs` should be modelled as a random slope - why? because it is a within-subject effect, you have many trials for this one slope
- Yes! they could definitely be correlated
""")

# ╔═╡ 5111d550-63a1-49b7-bb57-865eec80a8b4
md"""
### Implementation
Let's build it!
"""

# ╔═╡ c67afef2-1c34-475b-b282-65848a7a8376
#f = @formula(!!! ~ 1 + !!! + (1 + !!! | !!!))


# ╔═╡ ebcadd33-097b-4a0f-9a3d-3c970c962454
answer_box("`f = @formula(decisionTime ~ 1 + probability_abs + (1 + probability_abs | participant_id))`")

# ╔═╡ 76f41fb4-cc88-4684-a775-9ccf2db2fb5b
m1 = lmm(f,d_clean);

# ╔═╡ ee56201d-2c67-482c-b598-991676cd4c6d
MixedModelsMakie.coefplot(m1)

# ╔═╡ bc3c9627-92bd-41d7-90ef-2f612cb48204
PlutoTeachingTools.protip(
"""
Why are the scales of the two coefficients so different? What could you change **in your data / specification** to align them better?

*(obviously you could modify the plot with dual-axis, but that's just bad style)*
""",invite="Bonus",boxlabel="Question")

# ╔═╡ c5d438c5-9e7a-48db-9a73-e8ac3427713f
# ---

md"""
### Interpreting Coefficients

Let's extract some model parameters to better understand
"""

# ╔═╡ 754753fe-9904-4385-92c1-a81d82bad5e6
PlutoTeachingTools.question_box(md"""
  1. **Fixed effects**: What is the average effect of probability distance on decision time? 
  2. Is there more variability for the intercept or for the slope over subjects?
  3. **Correlation**: Are intercept and slope correlated?

  **Conceptual questions**:
  - What does the intercept represent in this experiment?
  - What does the sign of the slope tell you about the participants' decision-making?
  - What percentage of total variance is between participants vs within participants?

Remember the ICC for this model can be calculated by:

$$ICC =\frac{var_{between}}{var_{within} + var_{between}}$$

You can extract these variances 
- What does this value tell you?
  """)

# ╔═╡ 0200e609-dc87-4c4c-8c7d-ab8f0c0e0b30
PlutoTeachingTools.tip(md"""
  **Functions that might be helpful"
  - `fixef(m1)` - fixed effect estimates
  - `VarCorr(m1)` - variance components
  - `randeff(m1)` - individual participant random effects
  - `stderror(m1)` - standard errors of fixed effects
  """)

# ╔═╡ 2c4eece9-0fe5-44c4-ad53-825e89065482
md"""
### Calculate ICC
"""

# ╔═╡ 19cb2289-d245-4e86-91c8-715870238b6c
 #vc = XXX #Variance/Covariance Structure


# ╔═╡ 7c5abf44-3e6f-48b3-ae53-6e99120871a5
answer_box("`vc =VarCorr(m1)`")

# ╔═╡ d6be21ea-f8a0-4e13-af22-3d94886ca14c
begin

  #sigmas = values(vc.σρ[:!!!][:σ])
  #var_intercept = sum(sigmas.^2)   # !!! → group name, component index
  #var_residual = vc.!!! ^ 2            # !!! → residual variance attribute

end

# ╔═╡ dfd992b1-d7b3-485a-ad45-0bd2895a7fc3
answer_box("""
```julia
		   
  var_between = sum(values(vc.σρ[:participant_id][:σ]).^2)
  var_within = vc.s^2
...
# ╔═╡ 1bdc0ed4-ea04-45cd-abaf-b529505b66b6
md"""
Let's compare to ICC from MixedModelsExtras.jl
"""

# ╔═╡ 098342b3-15dd-4a1a-80e7-a77323d20ca8
#ICC_mme = MixedModelsExtras.icc(m1)

# ╔═╡ 9a163bf6-ff9d-4ebe-a03e-3e37fb3eded1
# ---

md"""
## Exercise 3: Visualizing Random Effects

See how individual participants differ from the population.
"""

# ╔═╡ 8d19946a-77ba-4010-8d92-dfcc1ef59249
md"""
### Ranef
"""

# ╔═╡ e88b4eba-6b5f-4fbd-9ac8-60dad317fcfe
MixedModelsMakie.caterpillar(m1)

# ╔═╡ d67a52c9-3bb0-4694-a926-c5f78cbbf6f0
PlutoTeachingTools.question_box(md"""
Here we use `caterpillar(m1)` to visualize the random effects of all subjects

1) Both columns are sorted according to ...?
1) Why does the right column look sorted too?
1) What's up with the top three subjects?

""")

# ╔═╡ abd36462-0aad-4815-887d-edfe2a228fd7
md"""
### Shrinkage
"""

# ╔═╡ 82d5cdd2-3d59-4496-9241-08862f6d19b8
begin
  PlutoTeachingTools.keyconcept("Shrinkage", md"""
  Mixed models apply **shrinkage** (partial pooling): participant estimates with high uncertainty are pulled toward the population mean. This is typically more accurate than fitting each participant independently.
  """)
end

# ╔═╡ f3449e8a-7e16-4b1d-bc3a-a90c1ccd2db5
MixedModelsMakie.shrinkageplot(m1)

# ╔═╡ 0bf6653c-0a5d-4967-850d-001b08ec7704
question_box(md"""
1) Why do arrows have different length?
2) Does the model seem singular to you?
3) Are the random slopes dependent on eachother?
""")

# ╔═╡ e6b8ef2a-fa9b-4df5-9cad-0f0397f133aa
let
probability_abs = 0:10:50
	
int = ranef(m1)[1][1,:] .+ fixef(m1)[1] # BLUP intercepts
slo = ranef(m1)[1][2,:] .+ fixef(m1)[2] # BLUP slopes
single_subject_effects = int .+ kron(slo, probability_abs')
	
f,ax,h = series(probability_abs,single_subject_effects,solid_color=(:black,0.3))
d = effects(Dict(:probability_abs => 0:10:50),m1) # average effect
	
lines!(d.probability_abs,d.decisionTime,linewidth=10)
	f
end

# ╔═╡ c9524dde-ece0-4e41-8136-1decacb8c447
question_box(
"""
This plot helps with interpretation of the random correlation: What is the meaning of the ρ (rho/correlation)- relationship between the intercept and the slope?
"""
)

# ╔═╡ ed80e196-1156-4eb0-98fa-30626b44fb85
# ---

md"""
## Exercise 3: Model Comparison

Let's compare our results with a model comparison of the fixed effect of `probability_abs`
"""

# ╔═╡ 7b8372bf-e845-48ac-9542-699655c14ae2
PlutoTeachingTools.question_box(md"""
  1. **Null model**: Only random effects (no fixed effect of probability distance)
  2. **Full model**: use `m1` from above

  Use `lrtest(null_model, full_model)` to compare them.

  **Conceptual question**: Should the random effect structure stay the same between the two models?
  """)

# ╔═╡ 6dceaea6-bb7b-4c15-b42e-8ffb483d3291
begin

  #m0 = lmm(@formula(decisionTime ~ !!!, data_cean)
  #lrtest(!!!, !!!)

end

# ╔═╡ 85c6412f-60e7-4d06-ba80-e00b842d88ca
hint("""  1. Fit the null model (remove `probability_abs` from fixed effects)
  2. Keep the random effects `(1 + probability_abs | participant_id)` identical
  3. Use `lrtest(m0, m1)` for the likelihood ratio test
""")

# ╔═╡ 9839a937-f881-4966-aad6-e33fcb1a966e
answer_box(md"""
  ```julia
  m0 = lmm(@formula(decisionTime ~ 1 + (1 + probability_abs | participant_id)), d_clean)
  lrtest(m0, m1)
  ```
  """)

# ╔═╡ 95199d95-9e18-44ae-a6b4-360440d15c2a
# ---

md"""
**Interpret your result**:
- Is the p-value different to the Walds-t from above?
- Does adding `probability_abs` significantly improve the model?
"""

# ╔═╡ 35cfca3d-41fd-4582-a27c-c76241070051
# ---

md"""
## Exercise 4: Checking Assumptions

Mixed models assume:
1. **Residuals** are normally distributed
2. **Random effects** are normally distributed

Let's check both using QQ plots.
"""

# ╔═╡ 7da025e4-1789-4148-8ae3-3b68be3745b9
PlutoTeachingTools.question_box(md"""
Create QQ plots for:
  1. The model residuals
  2. The random intercept BLUPs
  3. The random slope BLUPs

  Do the points follow the diagonal line?

  **Conceptual question**: What would systematic deviation from the diagonal tell you about the model?
  """)

# ╔═╡ fa7ca3ee-6430-439a-a68a-df800297394a
qqplot(ranef(m1)[1][1,:];distribution=Normal,qqline=:fitrobust)

# ╔═╡ bc18b1be-30dc-4fb1-8fa3-432740148e45
qqplot(ranef(m1)[1][2,:];distribution=Normal,qqline=:fitrobust)

# ╔═╡ cdc57ada-875f-497f-8ead-1bcf5d9d0749
qqplot(residuals(m1);distribution=Normal,qqline=:fitrobust)

# ╔═╡ 2e0b5bef-bdd4-4f89-b107-15350e97a633
answer_box(md"""
**Interpretation**: If the points follow the diagonal line, the normality
assumption is reasonable. Deviations at the tails might indicate outliers
or non-normality.

**What if assumptions are violated?**
  - Non-normal residuals: Try transformations (log, sqrt)
  - Outliers in BLUPs: Check for data entry errors or genuine individual differences
  - Accept limitations and interpret accordingly!
""")
...
md"""
## Exercise 5: Interaction Effects

In our experiment, the arrangement of the display (Ordered vs Random) might
interact with probability distance. Does a more organized display make
probability information easier to extract?
"""

# ╔═╡ e5a357c8-72a5-415b-b35d-1d8c539ff8c7
PlutoTeachingTools.question_box(md"""
Fit the following three models:

  1. **Both Main effects only** (`probability_abs` and `arrangement`)
  2. **Main Effects + Interaction**

To fit an interaction you can use `@formula(y~ A + B + A&B)` (in R it would be `A:B`), or the shortcut `@formula(A*B)`


Don't forget to add the random slopes!
								
**Conceptual question**: What does a significant interaction mean in this context?
  """)

# ╔═╡ ac944cf9-39fc-4490-876e-e2c28d981c27
begin
	#m_main = lmm(@formula(!!! ~ 1 + !!! + (!!! | !!!)), d_clean)
	#m_int = lmm(@formula(!!! ~ 1 + !!! + (!!! | !!!)), clean)

end

# ╔═╡ 0ca60a37-5123-414a-bcc1-5cdd2422a1b5
answer_box("""
```julia
	   
m_main = lmm(@formula(decisionTime ~ 1 + arrangement + probability_abs + (1+ arrangement + probability_abs | participant_id)), d_clean)
m_int = lmm(@formula(decisionTime ~ 1 + arrangement * probability_abs + (1+ arrangement * probability_abs | participant_id)), d_clean)
```
""")
...
begin
    #  eff = effects(Dict(:probability_abs => !!!, :arrangement => !!!), m_int)
    
  data(eff) * mapping(:probability_abs, :decisionTime, color=:arrangement) * visual(ScatterLines) |> draw
...
question_box("""
1. Fix the plotting code. 
2. Do you observe an interaction of arrangement and probability_abs?
""")

# ╔═╡ 6a0e329c-83ae-40dd-8205-69f3d45b2daa
answer_box("""
		     ```
		   eff = effects(Dict(:probability_abs => 0:10:50, :arrangement => ["Ordered", "Random"]), m_int)
		   ```
		   """)
...
question_box("""
Now we use LRT to test the interaction.
...
# ╔═╡ 90d2e315-bd8c-4d1f-a599-e2c859f90cf6
answer_box("`lrtest(m_int, m_main)`")

# ╔═╡ 59b58890-f272-4c54-a151-3f39bf7e2cae
md"""
## Exercise 6: Different categorical coding schemes
...
# ╔═╡ 39f17e22-fc79-43ed-b95b-e513f1989a7c
answer_box("""
As discussed in the lecture, the intercept always represents what happens when everything is 0. This does not only mean the fixed effects, but also the random effects - effectively you are testing what happens if the most average subject (all random effects are 0) would take part in your study!

You could try to calculate it by subsetting the data for all predictors to 0 (e.g. the reference group for arrangement, 0 for probability_abs etc.), but likely the exact number will be different, because subjects have various number of trials etc. - not straight forward to calculate as from an linear model
""")
...
# lmm(f_dummy, d_clean; contrasts=!!!)

# ╔═╡ 51ffe48c-915f-4f55-bb08-5cac11fc6da3
answer_box("""
...
# ╔═╡ 60bb2cac-bfdc-480d-a47d-f10dac2f6caa
answer_box("""
...
# ╔═╡ 56ef29b9-6fd4-4e4a-becb-5bd88871fcb9
#f_noitem = @formula decisionTime ~ ... 

# ╔═╡ 4b7e6190-e089-448c-be92-8c9b9ba02539
PlutoTeachingTools.answer_box("""
- Fixed effects should be all factors you want to model, where you dont want to assume that they are 0. E.g. here `iconicity` & `arrangement`
- Random slopes should be everything that varies within a random grouping variable. 
							  
Thus for **subject:** `arrangement` and `iconicity`, as each subject sees multiple levels of both.

For **item**: only only `arrangement` makes sense, because we have the same pictures of houses in both levels, but we dont have the same houses in `iconicity`.
		   
""")

# ╔═╡ 83e50242-88ee-4940-a6e7-0d00e2aa7de8
answer_box("""
```julia
f_item = @formula decisionTime ~ 1 + arrangement * iconicity + (1 + arrangement * iconicity | participant_id) + (1+arrangement|item)
		   
f_noitem = @formula decisionTime ~ 1 + arrangement * iconicity + (1 + arrangement * iconicity | participant_id)
```
""")
...
# ╔═╡ e6fe85c2-be00-48cb-a135-c7c862b83b60
answer_box("""
Clearly, decision time cannot be below zero. Also decision time should be log-normal, not normal as in these simulations. So no; not very realistic ;)
""")
...
begin
    m_item = lmm(f_item, d_sim)
    m_noitem = lmm(f_noitem, d_sim)
end
...
# ╔═╡ 9af4b1ce-71e2-4852-8ae1-6fa08119e985
answer_box("""
```
...
m_subset = lmm(@formula(!!! ~!!! + (!!!|!!!)),d_subset)

# ╔═╡ aa5282ce-6c8b-48dc-a3d1-e4480666a384
answer_box("`m_subset = lmm(@formula(decisionTime ~ arrangement*probability_abs + (1+arrangement*probability_abs|participant_id)),d_subset)`")
...
# ╔═╡ 0032328d-915a-4e52-98a3-e09051a0c4ec
answer_box("`MixedModels.rePCA(m_subset)`")

# ╔═╡ eb8ee8b6-8cb0-4cbb-8132-dd512d7cb661
# m_subset_simplified = lmm(@formula(decisionTime ~ !!!),d_subset)


# ╔═╡ 644cf4e9-5e9e-4c36-9c79-a5cb7d3d859b
answer_box("`m_subset_simplified = lmm(@formula(decisionTime ~ arrangement*probability_abs + (1+arrangement*probability_abs|participant_id)),d_subset)`")

# ╔═╡ 45dcfe36-ebe0-4773-b4f5-bdda6641dbaa
#VarCorr(!!!)


# ╔═╡ 2a052bc8-81f0-483d-b4fa-f9fcb85f94c3
answer_box("""```VarCorr(m_subset)```""")

# ╔═╡ 95dc2cf3-3b07-4438-95a2-8f5f857b3107
# MixedModels.rePCA(!!!)


# ╔═╡ de05132a-8a56-4fec-93af-c685bdcad997
answer_box("`MixedModels.rePCA(m_subset_simple)`")
...
# ╔═╡ 738a80f1-7b16-465a-9a2a-d676558a43b8
# lrtest(!!!,!!!)
...
# ╔═╡ b8df169e-a8a4-487c-b1c1-55b384900b37
md"""
# Setup
"""

# ╔═╡ 939fdd7e-d43c-4153-9095-f8a42d87d405
display_as(x) = DisplayAs.Text(x)

# ╔═╡ ece51acc-915c-4844-9023-e45feb3c0ea5
m1 |> display_as
...
Effects = "~1.7.1"
MixedModels = "~5.8.2"
MixedModelsExtras = "~2.3.2"
MixedModelsMakie = "~0.4.12"
...
manifest_format = "2.0"
project_hash = "6bc5c2f3ecbfc2b8ae008d4270d58c197ccf946f"
...
deps = ["GLM", "LinearAlgebra", "MixedModels", "Statistics", "StatsBase", "StatsModels", "Tables"]
git-tree-sha1 = "f17f8b712cdcd394fd94f24c3a4f02182aa91152"
...
git-tree-sha1 = "962e92643b475919b0fdc82af92a045c72ce01c3"
...
[[deps.SwarmMakie]]
deps = ["KernelDensity", "Makie", "Random", "StableRNGs", "StatsBase"]
git-tree-sha1 = "345288cc718690340ef362fc717b94a1958a9f06"
